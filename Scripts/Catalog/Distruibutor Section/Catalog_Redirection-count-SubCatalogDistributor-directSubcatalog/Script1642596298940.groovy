...
//Get Sub Catalog Catalog ID from The element direct tile Sub catalog
...
if (Maintilelength == 1) {
        ...
        GlobalVariable.catalog_type1 = "direct"
}
...
int sizeofsubcatalog = WebUI.findWebElements(findTestObject('Object Repository/Catalog/SubCatalogs/ParamertizedSubCatalog - withoutIndex', [('tiletype'): GlobalVariable.catalog_type1]), 2).size()
...
for (int m = 1; m <= sizeofsubcatalog; m++) {
        WebElement direct_sub_catalog_id = WebUiCommonHelper.findWebElement(findTestObject('Object Repository/Catalog/SubCatalogs/ParamertizedSubCatalog', [('tiletype'): GlobalVariable.catalog_type1, ('position'): m]), 10)

        WebElement Sub_catalog_ids = direct_sub_catalog_id
        String sub_catalog_id = Sub_catalog_ids.getAttribute('data-catalog-id')

        KeywordUtil.logInfo("Loop Position ${m}")

        KeywordUtil.logInfo("Entered in Catalog tile loop")
        ...
        WebUI.click(findTestObject('Object Repository/Catalog/SubCatalogs/ParamertizedSubCatalog', [('tiletype'): GlobalVariable.catalog_type1, ('position'): m]))
...

        {
                ...
                KeywordUtil.logInfo("Product List Count ${distributorid}")
                ...
                if (ProductcountfinalInteger == databasecount) {
                ...
                for (int product_type_length = 1; product_type_length <= sizeoftps; product_type_length++) {
						...
						GlobalVariable.final_tps_ids = final_tps_ids
                        ...
                        def getpricefromsub_catalog = "select ticket_id, ticketpriceschedule_id, ROUND(ticket_list_price,2) as ticket_list_price, ROUND(ticket_new_price,2) as ticket_new_price, ROUND(museum_gross_commission,2) as museum_gross_commission from channel_level_commission where catalog_id = " + sub_catalog_id + " and ticketpriceschedule_id = " + final_tps_ids + " and is_adjust_pricing = '1'"

                        def getpricefrom_distributor = "select ticket_id, ticketpriceschedule_id, ROUND(ticket_list_price,2) as ticket_list_price, ROUND(ticket_new_price,2) as ticket_new_price, ROUND(museum_gross_commission,2) as museum_gross_commission from ticket_level_commission where hotel_id = " + distributorid + " and ticketpriceschedule_id = " + final_tps_ids + " and is_adjust_pricing = '1'"

                        KeywordUtil.logInfo(" Query to get Prices from clc :::: ${getpricefromclc}")
                        KeywordUtil.logInfo(" Query to get Prices from Subcatalog :::: ${getpricefromsub_catalog}")
                        ...
                        queryforsubcatalog = CustomKeywords.
                        'com.katalon.plugin.keyword.connection.DatabaseKeywords.executeQuery'(globalConnection, "$getpricefromsub_catalog")
                        ...
                        'Example: check result set is empty'
                        println CustomKeywords.
                        'com.katalon.plugin.keyword.connection.ResultSetKeywords.isEmptyResult'(queryforsubcatalog)
                        ...
                        if (CustomKeywords.
                                'com.katalon.plugin.keyword.connection.ResultSetKeywords.isEmptyResult'(queryfordistributor) == true) {

                                if (CustomKeywords.
                                        'com.katalon.plugin.keyword.connection.ResultSetKeywords.isEmptyResult'(queryforsubcatalog) == true) {
                                ...
                                if (CustomKeywords.
                                        'com.katalon.plugin.keyword.connection.ResultSetKeywords.isEmptyResult'(queryforsubcatalog) == false) {

                                        def ticket_id_clc = CustomKeywords.
                                        'com.katalon.plugin.keyword.connection.ResultSetKeywords.getSingleCellValue'(queryforsubcatalog, 1, 'ticket_id')

                                        def ticketpriceschedule_id_clc = CustomKeywords.
                                        'com.katalon.plugin.keyword.connection.ResultSetKeywords.getSingleCellValue'(queryforsubcatalog, 1, 'ticketpriceschedule_id')

                                        def ticket_list_price_clc = CustomKeywords.
                                        'com.katalon.plugin.keyword.connection.ResultSetKeywords.getSingleCellValue'(queryforsubcatalog, 1, 'ticket_list_price')
                                        Float ticket_list_price_float_clc = Float.parseFloat(ticket_list_price_clc)
                                        println(ticket_list_price_float_clc)

                                        def ticket_new_price_clc = CustomKeywords.
                                        'com.katalon.plugin.keyword.connection.ResultSetKeywords.getSingleCellValue'(queryforsubcatalog, 1, 'ticket_new_price')
                                        Float ticket_new_price_float_clc = Float.parseFloat(ticket_new_price_clc)
                                        println(ticket_new_price_float_clc)

                                        def museum_gross_commission_clc = CustomKeywords.
                                        'com.katalon.plugin.keyword.connection.ResultSetKeywords.getSingleCellValue'(queryforsubcatalog, 1, 'museum_gross_commission')
                                        Float museum_gross_commission_float_clc = Float.parseFloat(museum_gross_commission_clc)
                                        println(museum_gross_commission_float_clc)
                                        ...
                                        if (list_price_float == ticket_list_price_float_clc) {
                                                ...
                                                KeywordUtil.logInfo(" Listing Price Matched if entry Exist in Sub catalog :::: View::  -- TPS List Price :: ")

                                        } else {

                                                KeywordUtil.logInfo(" Entry Exist in Sub catalog and List Price with TPS not matched")
                                        }

                                        if (salesPrice_float == ticket_new_price_float_clc) {

                                                KeywordUtil.logInfo(" Sale Price Matched if entry Exist in Sub catalog :::: View:: -- TPS List Price :: ")

                                        } else {

                                                KeywordUtil.logInfo(" Entry Exist in Sub catalog and Sale Price with TPS not matched")
                                        }

                                        if (purchase_float == museum_gross_commission_float_clc) {

                                                KeywordUtil.logInfo(" Purchase Matched if entry Exist in Sub catalog :::: View:: -- TPS List Price :: ")

                                        } else {

                                                KeywordUtil.logInfo(" Entry Exist in Sub Catalog and Purchase Price with TPS not matched")
                                        }

                                        if (margin_float == (ticket_new_price_float_clc - museum_gross_commission_float_clc)) {

                                                KeywordUtil.logInfo(" Margin Matched if entry Exist in Sub Catalog :::: View:: -- TPS List Price :: ")

                                        } else {

                                                KeywordUtil.logInfo(" Entry  Exist in Sub catalog and Margin with TPS not matched")
                                        }
						...
						WebUI.callTestCase(findTestCase('Test Cases/Catalog/Distruibutor Section/Catalog_Redirection-count-SubCatalogDistributor-Distributor_query'), [: ], FailureHandling.CONTINUE_ON_FAILURE)
...

        {
                ...
                KeywordUtil.logInfo("Distributor Not Exist in the Catalog Id = ${catalog_id}")
