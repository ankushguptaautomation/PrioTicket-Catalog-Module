...
def supplierprices_query2 = "SELECT ticket_gross_price as saleprice FROM ticket_level_commission where hotel_id = " + "'" + distributorid + "'" + " and ticketpriceschedule_id = " + "'" + GlobalVariable.gv_tps_id + "'" + " and resale_currency_level = '1'"
KeywordUtil.logInfo("${supplierprices_query2}")
queryfortlc = CustomKeywords.
'com.katalon.plugin.keyword.connection.DatabaseKeywords.executeQuery'(globalConnection, "$supplierprices_query2")

if (CustomKeywords.
        'com.katalon.plugin.keyword.connection.ResultSetKeywords.isEmptyResult'(queryfortlc) == false) {
...

        {
                ...
                def supplierprices_query = "SELECT ticket_gross_price as saleprice FROM ticket_level_commission where hotel_id = " + "'" + distributorid + "'" + " and ticketpriceschedule_id = " + "'" + GlobalVariable.gv_tps_id + "'" + " and resale_currency_level = '1'"
                ...
                if (saleprice_fromdb == supplier_sale_price_float) {
                        ...
                        KeywordUtil.logInfo("Account level Sale Price from the Supplier row matched with view and database")

                } else {

                        KeywordUtil.logInfo("Account level Sale Price from the Supplier row NOT Matched with view and database")
                ...
                if (resaleprice_fromdb == supplier_resale_price_float) {
                        ...
                        KeywordUtil.logInfo("Account Level ReSale Price from the Supplier row matched with view and database")

                } else {

                        KeywordUtil.logInfo("Account Level ReSale Price from the Supplier row NOT Matched with view and database")
                ...
                if (purchaseprice_fromdb == supplier_purchase_price_float) {
                        ...
                        KeywordUtil.logInfo("Account level Purchase Price from the Supplier row matched with view and database")

                } else {

                        KeywordUtil.logInfo("Account Level Purchase Price from the Supplier row NOT Matched with view and database")
                ...
                if (reseller_commission_fromdb == reseller_commission_float) {
                        ...
                        KeywordUtil.logInfo("Account Level reseller_commission from the Supplier row matched with view and database")

                } else {

                        KeywordUtil.logInfo("Account Level reseller_commission from the Supplier row NOT Matched with view and database")
...

        {
                ...
                def supplierprices_query = "SELECT Round(ticket_gross_price,2) as saleprice, ROUND((hgs_commission_net_price*(100+ticket_tax_value)/100),2) as reseller_margin, ROUND((hotel_commission_net_price*(100+ticket_tax_value)/100),2) as distributor_commission, ROUND((museum_gross_commission),2) as product_cost, ROUND(ticket_gross_price-hotel_commission_net_price*(100+ticket_tax_value)/100,2) as resale_price, hgs_prepaid_commission_percentage FROM ticket_level_commission clc join (SELECT channel_id, ticketpriceschedule_id, max(resale_currency_level) as resale_currency_level FROM ticket_level_commission where hotel_id = " + "'" + distributorid + "'" + " and ticketpriceschedule_id = " + "'" + GlobalVariable.gv_tps_id + "'" + " and deleted = 0 and resale_currency_level in (1,2)) as base on clc.channel_id = base.channel_id and clc.ticketpriceschedule_id = base.ticketpriceschedule_id and clc.resale_currency_level = base.resale_currency_level"
                ...
                if (saleprice_fromdb == supplier_sale_price_float) {
                        ...
                        KeywordUtil.logInfo("Account Level Sale Price from the Base row matched with view and database")

                } else {

                        KeywordUtil.logInfo("Account Level Sale Price from the Base row NOT Matched with view and database")
                ...
                if (resaleprice_fromdb == supplier_resale_price_float) {
                        ...
                        KeywordUtil.logInfo("Account Level ReSale Price from the Base row matched with view and database")

                } else {

                        KeywordUtil.logInfo("Account Level ReSale Price from the Base row NOT Matched with view and database")
                ...
                if (purchaseprice_fromdb == supplier_purchase_price_float) {
                        ...
                        KeywordUtil.logInfo("Account Level Purchase Price from the Base row matched with view and database")

                } else {

                        KeywordUtil.logInfo("Account level Purchase Price from the Base row NOT Matched with view and database")
                ...
                if (reseller_commission_fromdb == reseller_commission_float) {
                        ...
                        KeywordUtil.logInfo("Account Level reseller_commission from the Base row matched with view and database")

                } else {

                        KeywordUtil.logInfo("Account Level reseller_commission from the Base row NOT Matched with view and database")
...

        {
                ...
                def supplierprices_query = "SELECT Round(ticket_gross_price,2) as saleprice, ROUND((hgs_commission_net_price*(100+ticket_tax_value)/100),2) as reseller_margin, ROUND((hotel_commission_net_price*(100+ticket_tax_value)/100),2) as distributor_commission, ROUND((museum_gross_commission),2) as product_cost, ROUND(ticket_gross_price-hotel_commission_net_price*(100+ticket_tax_value)/100,2) as resale_price, hgs_prepaid_commission_percentage FROM ticket_level_commission clc join (SELECT channel_id, ticketpriceschedule_id, max(resale_currency_level) as resale_currency_level FROM ticket_level_commission where hotel_id = " + "'" + distributorid + "'" + " and ticketpriceschedule_id = " + "'" + GlobalVariable.gv_tps_id + "'" + " and deleted = 0 and resale_currency_level in (1,2,3)) as base on clc.channel_id = base.channel_id and clc.ticketpriceschedule_id = base.ticketpriceschedule_id and clc.resale_currency_level = base.resale_currency_level"
                ...
                if (saleprice_fromdb == supplier_sale_price_float) {
                        ...
                        KeywordUtil.logInfo("Account Level Sale Price from the New currency row matched with view and database")

                } else {

                        KeywordUtil.logInfo("Account Level Sale Price from the New currency row NOT Matched with view and database")
                ...
                if (resaleprice_fromdb == supplier_resale_price_float) {
                        ...
                        KeywordUtil.logInfo("Account Level ReSale Price from the New currency row matched with view and database")

                } else {

                        KeywordUtil.logInfo("Account Level ReSale Price from the New currency row NOT Matched with view and database")
                ...
                if (purchaseprice_fromdb == supplier_purchase_price_float) {
                        ...
                        KeywordUtil.logInfo("Account Level Purchase Price from the New currency row matched with view and database")

                } else {

                        KeywordUtil.logInfo("Account Level Purchase Price from the New currency row NOT Matched with view and database")
                ...
                if (reseller_commission_fromdb == reseller_commission_float) {
                        ...
                        KeywordUtil.logInfo("Account Level reseller_commission from the New currency row matched with view and database")

                } else {

                        KeywordUtil.logInfo("Account Level reseller_commission from the New currency row NOT Matched with view and database")
