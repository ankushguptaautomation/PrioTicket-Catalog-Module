...
//for (int n = GlobalVariable.lengthh; n <= GlobalVariable.gv_sizeofsubcatalog; n++) {

if (CustomKeywords.
	'com.katalon.plugin.keyword.connection.ResultSetKeywords.isEmptyResult'(queryforsub_catalog) == false) {

	KeywordUtil.logInfo("Record Exist In Sub catalog")
...

	{
			...
			def supplierprices_query = "SELECT ticket_gross_price as saleprice FROM channel_level_commission where catalog_id = " + "'" + GlobalVariable.GV_sub_catalog_id + "'" + " and ticketpriceschedule_id = " + "'" + GlobalVariable.gv_tps_id + "'" + " and resale_currency_level = '1'"
...

	{
			...
			def supplierprices_query = "SELECT Round(ticket_gross_price,2) as saleprice, ROUND((hgs_commission_net_price*(100+ticket_tax_value)/100),2) as reseller_margin, ROUND((hotel_commission_net_price*(100+ticket_tax_value)/100),2) as distributor_commission, ROUND((museum_gross_commission),2) as product_cost, ROUND(ticket_gross_price-hotel_commission_net_price*(100+ticket_tax_value)/100,2) as resale_price, hgs_prepaid_commission_percentage FROM channel_level_commission clc join (SELECT channel_id, ticketpriceschedule_id, max(resale_currency_level) as resale_currency_level FROM channel_level_commission where catalog_id = " + "'" + GlobalVariable.GV_sub_catalog_id + "'" + " and ticketpriceschedule_id = " + "'" + GlobalVariable.gv_tps_id + "'" + " and deleted = 0 and resale_currency_level in (1,2,3)) as base on clc.channel_id = base.channel_id and clc.ticketpriceschedule_id = base.ticketpriceschedule_id and clc.resale_currency_level = base.resale_currency_level"
