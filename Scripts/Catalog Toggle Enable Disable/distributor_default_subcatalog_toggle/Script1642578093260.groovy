...
List<WebElement> direct_subcatalog_name = WebUiCommonHelper.findWebElements(findTestObject('1) Catalog Exports Objects/Distributor Section/distributor_sub_catalog'), 10)


	for (int k = 0; k < direct_subcatalog_name.size(); k++)
		{
			WebElement el1=direct_subcatalog_name.get(k)
			String direct_subcatalog_delete =el1.getAttribute('data-catalog-name')
			direct_subcatalog_id_delete =el1.getAttribute('data-catalog-id')
			direct_subcatalog_id_accounts =el1.getAttribute('data-cod-id')
			...
			println("Direct Subcatalog name : "+el1.getAttribute('data-catalog-name'))
			println("Direct Subcatalog ID : "+el1.getAttribute('data-catalog-id'))
			...
			if(direct_subcatalog_delete==GlobalVariable.delete_subcatalog)
				{
					...
					WebUI.findWebElements(findTestObject('1) Catalog Exports Objects/Distributor Section/distributor_sub_catalog'),2).get(k).click()
					log.logInfo("Direct Sub catalog Tile Section Clicked Successfully")
...
//WebUI.click(findTestObject('1) Catalog Exports Objects/Direct Section/direct_sub_catalog'))
//log.logInfo("Direct Sub catalog Tile Section Clicked Successfully")
//
//WebUI.delay(5)
...
for (int i = 0; i < toggle.size(); i++) 
	{
		...
		if (WebUI.verifyElementNotChecked(findTestObject('Object Repository/Catalog Toggle Enable Disable Objects/toggle_button_listing'), 10,FailureHandling.OPTIONAL)==true) 
			...
			query="select is_pos_list from template_level_tickets where ticket_id="+product_id+" and catalog_id='"+direct_subcatalog_id_delete+"' and deleted='0' and template_id ="+sec_temp
			println("Query : "+query)
			...
			if (CustomKeywords.'com.katalon.plugin.keyword.connection.ResultSetKeywords.isEmptyResult'(catalogs) == true) {
...
//			println("Hotel LIST IN DATABASE : "+relace_database_hotel_list12)
		...
		{
			println("NO SUCH CATALOG EXIST")
			log.logInfo("NO SUCH CATALOG EXIST")
		}
