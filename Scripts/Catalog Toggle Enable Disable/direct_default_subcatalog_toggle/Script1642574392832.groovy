			...
			if(direct_subcatalog_delete==GlobalVariable.delete_subcatalog)
				{
					...
					WebUI.findWebElements(findTestObject('1) Catalog Exports Objects/Direct Section/direct_sub_catalog'),2).get(k).click()
...
for (int i = 0; i < toggle.size(); i++) 
	{
		...
		if (WebUI.verifyElementNotChecked(findTestObject('Object Repository/Catalog Toggle Enable Disable Objects/toggle_button_listing'), 10,FailureHandling.OPTIONAL)==true) 
			...
			query="select is_pos_list from template_level_tickets where ticket_id="+product_id+" and catalog_id='"+direct_subcatalog_id_delete+"' and deleted='0' and template_id ="+primary_temp
			println("Query : "+query)
