...
String message
String direct_subcatalog_id_accounts
...
//Delay of 2 seconds
WebUI.delay(2)
String direct_subcatalog_id_delete
String database_deleted_status

//getting count of subcatalog
int direct_subcatalog=WebUI.findWebElements(findTestObject('1) Catalog Exports Objects/Direct Section/direct_sub_catalog'),2).size()
println("No.of Direct Subcatalogs:" +direct_subcatalog);
log.logInfo("No.of Direct Subcatalogs:" +direct_subcatalog)
...
List<WebElement> direct_subcatalog_name = WebUiCommonHelper.findWebElements(findTestObject('1) Catalog Exports Objects/Direct Section/direct_sub_catalog'), 10)
	...
	for (int i = 0; i < direct_subcatalog_name.size(); i++) 
		{
			...
			direct_subcatalog_id_delete =el.getAttribute('data-catalog-id')
			direct_subcatalog_id_accounts =el.getAttribute('data-cod-id')
			
			if (direct_subcatalog_id_accounts =="")
				{
					direct_subcatalog_id_accounts = '0'
				}
			else
				{
					direct_subcatalog_id_accounts = direct_subcatalog_id_accounts;
				}
			
			println("Direct Subcatalog name : "+el.getAttribute('data-catalog-name'))
			println("Direct Subcatalog ID : "+el.getAttribute('data-catalog-id'))
			println("Direct Subcatalog ID Accounts Linked : "+direct_subcatalog_id_accounts)
			
			
	
			if(direct_subcatalog_delete==GlobalVariable.delete_subcatalog)
				{
					WebUI.findWebElements(findTestObject('Object Repository/Catalog Delete SubCatalog Objects/direct_more_action_button'),2).get(i).click()
			
					if(WebUI.verifyElementPresent(findTestObject('Object Repository/Catalog Delete SubCatalog Objects/direct_subcatalog_plus_focus'), 5, FailureHandling.CONTINUE_ON_FAILURE)==true)
						{

							List<WebElement> direct_listing = WebUiCommonHelper.findWebElements(findTestObject('Object Repository/Catalog Delete SubCatalog Objects/direct_plus_button_listing'), 10)
							for (int j = 1; j < direct_listing.size(); j++)
								{
									...
									if(direct_subcatalog_delete_button=="Delete")
										{
											
											WebUI.findWebElements(findTestObject('Object Repository/Catalog Delete SubCatalog Objects/direct_plus_button_listing'),2).get(j).click()
											WebUI.click(findTestObject('Object Repository/Catalog Delete SubCatalog Objects/confirm_delete_button'))
											println("Subcatalog ID which deleted from Catalogs : "+direct_subcatalog_id_delete)
											
											WebUI.delay(5)
											
											//Query to find count of Pending Products in Database
											query="select is_deleted from catalogs where catalog_id = "+direct_subcatalog_id_delete
											println("Query : "+query)
														
											qr_codes_query="Select sub_catalog_id from qr_codes where cod_id in ("+direct_subcatalog_id_accounts+")"
											...
											println("DELETED STATUS IN DATABASE : "+database_deleted_status)
												...
												if(database_deleted_status=='1')
													...
													{
														println("ACCOUNTS UNLINKED FROM SUBCATALOG ,Account ID's :"+direct_subcatalog_id_accounts)
													}
													else
													{
	...
	List<WebElement> direct_subcatalog_name_after_delete = WebUiCommonHelper.findWebElements(findTestObject('1) Catalog Exports Objects/Direct Section/direct_sub_catalog'), 10)
	
		for (int k = 0; k < direct_subcatalog_name_after_delete.size(); k++)
			{
				WebElement el12=direct_subcatalog_name_after_delete.get(k)
				String direct_subcatalog_name1 =el12.getAttribute('data-catalog-name')
				String direct_subcatalog_id =el12.getAttribute('data-catalog-id')
				...
				while(itr.hasNext())
					...
					if(itr.hasNext()==direct_subcatalog_id_delete)
						{
						...
						{
							println("CATALOG ID DELETED AFTER DELETED FROM FRONTEND "+direct_subcatalog_id_delete)
						}
					}
			}
			...
			product_list_query="select DISTINCT (mec.mec_id) as ticket_id from modeventcontent mec LEFT JOIN qr_codes qc on mec.cod_id=qc.cod_id and qc.service_name=1 LEFT JOIN ticketpriceschedule tp on tp.ticket_id = mec.mec_id and tp.default_listing = '1' and tp.deleted = '0' and ((tp.start_date <= '"+current_time+"' AND tp.end_date >= '"+current_time+"') OR tp.start_date > '"+current_time+"') LEFT JOIN channel_level_commission clc on mec.mec_id=clc.ticket_id and clc.channel_id = "+primary_channel+" and (clc.ticketpriceschedule_id = '' or clc.ticketpriceschedule_id = '0' or clc.ticketpriceschedule_id = tp.id)JOIN template_level_tickets tlt on mec.mec_id = tlt.ticket_id and tlt.template_id = "+primary_temp+" where mec.deleted='0' and mec.parent_ticket_id = '0' and tp.deleted = '0' AND tlt.deleted = '0' and tlt.publish_catalog = '1' and (mec.is_commission_assigned='1' or tp.is_commission_assigned='1') and ( tlt.is_pos_list='1' || tlt.is_pos_list='0' )"
			...
			String query_pos_data="select concat('--Ticket_id-- : ',pos.mec_id,' --Hotel ID-- : ',pos.hotel_id,' --POS PosList-- : ',pos.is_pos_list,' --Template Pos List-- : ',tlt.is_pos_list) as Result, pos.mec_id , pos.hotel_id,pos.is_pos_list, tlt.is_pos_list  from pos_tickets pos LEFT join template_level_tickets tlt on pos.mec_id=tlt.ticket_id  where pos.mec_id in "+relace_database_product_list12+" and pos.hotel_id in ("+direct_subcatalog_id_accounts+") and tlt.template_id="+primary_temp+" and tlt.catalog_id='0' and tlt.is_pos_list!=pos.is_pos_list"
