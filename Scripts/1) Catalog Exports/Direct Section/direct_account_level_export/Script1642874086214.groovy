...
query_direct_account_id="SELECT cod_id FROM `qr_codes` where reseller_id="+reseller_id+" and own_supplier_id!='0' and cashier_type='1' and sub_catalog_id='0' and template_id="+primary_temp+" and channel_id="+primary_channel+" order by last_modified_at DESC Limit 1"
...
if (CustomKeywords.'com.katalon.plugin.keyword.connection.ResultSetKeywords.isEmptyResult'(database_account_id) == true)
	...
	println("No Direct Account Level Export Exist on Direct Tile Account Level Section")
		message="No Direct Account Level Export Exist on Direct Tile Account Level Section"
		...
		if(WebUI.verifyElementClickable(findTestObject('1) Catalog Exports Objects/account_level_export_csv'), FailureHandling.CONTINUE_ON_FAILURE)==true)
			{
				...
				println("Export Exist on Direct Account Level and able to export")
				WebUI.takeFullPageScreenshot()
				message="Export Exist on Direct Account Level and able to export"
				WebUI.delay(5)
			...
			{
				println("ERROR:Not Able to Export Direct Account Level")
				WebUI.takeFullPageScreenshot()
				message="ERROR:Not Able to Export Direct Account Level"
			}
	}
