...
//Getting Reseller ID from DB for Account Export
...
if (CustomKeywords.'com.katalon.plugin.keyword.connection.ResultSetKeywords.isEmptyResult'(database_account_id) == true)
	...
	println("No Reseller Account Level Export Exist on Reseller Tile Account Level Section")
		message="No Reseller Account Level Export Exist on Reseller Tile Account Level Section"
...
else
{
	...
	//Getting Count from Database and store in Variable
	 ...
	 log.logInfo("Reseller ID for Export : "+distributor_id)

//Select Reseller Tile
WebUI.click(findTestObject('1) Catalog Exports Objects/Reseller Section/reseller_main_catalog'))

//Reseller ID to searh in search box
WebUI.setText(findTestObject('1) Catalog Exports Objects/search_button'), distributor_id)
log.logInfo("Reseller ID for which CSV Exported : "+distributor_id)
WebUI.takeFullPageScreenshot()
...
if(WebUI.verifyElementPresent(findTestObject('1) Catalog Exports Objects/reseller_account_level_export_csv'), 5, FailureHandling.CONTINUE_ON_FAILURE)==true)
	{
		//Verify element selected is clickable or not
		if(WebUI.verifyElementClickable(findTestObject('1) Catalog Exports Objects/reseller_account_level_export_csv'), FailureHandling.CONTINUE_ON_FAILURE)==true)
			{
				WebUI.delay(3)
				//Reseller Export
				WebUI.click(findTestObject('1) Catalog Exports Objects/reseller_account_level_export_csv'))
				println("Export Exist on Reseller Account Level and able to export")
				WebUI.takeFullPageScreenshot()
				message="Export Exist on Reseller Account Level and able to export"
				WebUI.delay(5)
			...
			{
				println("ERROR:Not Able to Export Reseller Account Level")
				WebUI.takeFullPageScreenshot()
				message="ERROR:Not Able to Export Reseller Account Level"
			}
	}
